...
while True:
    ...
    if ret == True:
        ...
        #0:name 1:上側x軸 2:上側y軸 3:height 4,weidth 4:信頼度
            ...
            cv2.rectangle(
                    ...
                    pt2 = (int(yolo[i][1] + (len(yolo[i][0]) * 10) + 120),int(yolo[i][2])),
                    color = (color_red,color_green,color_blue),
                    thickness = -1
                    #以下略
                    )
            ...
            cv2.putText(
                    frame,
                    text = str(round(yolo[i][3] * yolo[i][4])),
                    org = (int(yolo[i][1] + (len(yolo[i][0] * 40))),int(yolo[i][2]) - 5) ,
                    fontFace = cv2.FONT_HERSHEY_PLAIN,
                    fontScale = 1.0,
                    color = (0,0,0),
                    thickness = 1,
